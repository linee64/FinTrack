...
export const ChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 1,
      text: 'Привет. Чем могу помочь с финансами?',
      sender: 'ai',
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const prompt = `Ты — финансовый эксперт-аналитик KAPITAL.AI (Казахстан). 

ДАННЫЕ:
Кредиты: ${loansContext || 'нет'}
Расходы: ${expensesContext || 'нет'}

ПРАВИЛА ОТВЕТА:
1. КОНКРЕТНОСТЬ И СУТЬ: Отвечай по существу на вопрос. Никаких общих фраз, лишних вступлений ("Я изучил ваши данные...") и заключений. Сразу к решению.
2. БАЛАНС КРАТКОСТИ: Ответ должен быть коротким, но информативным. Не ограничивайся одним предложением, если нужно объяснить суть, но избегай "воды".
3. ФОРМАТ: Предпочтительно использовать короткие списки или 3-5 предложений. Главное — концентрированная польза.
4. ВАЛЮТА: Только Тенге (₸). 
